...
class JsonHelpers
{
    ...
    public static bool KeyExists(string filePath, string username)
    {
        // TODO JSON
        ...
        if (existingData == "") {
            ...
            Dictionary<string, string> existingDataDictionary = JsonSerializer.Deserialize<Dictionary<string, string>>(existingData)
                                                               ?? throw new Exception(
                                    $"Thread {Thread.CurrentThread.ManagedThreadId}: JsonHelpers.KeyExists() dictionary was null");
    ...
    // NOTE - You must own the lock over the filePath
    public static void RemoveKeyPair(string Key)
    {
        // TODO JSON
        return;
    }
    ...
    // NOTE - You must own the lock over the filePath
    public static void AddKeyPair(string filePath, string key, string value)
    {
        string existingData = File.ReadAllText(filePath);
